...
export function createTransitState()                     {
  return {
...
export function processTransitMovement(
  ...
  const currentTransit                     = transit ? { ...transit } : createTransitState();
  const nextCarried            = { ...carried };
  const dropsGained                        = [];
  const eventsVi           = [];
  let pointsGained = 0;
  let scavengeCount = 0;
...
function pickRandomTransitDrop()                      {
 ...
 */
export function canCollectOutpost(transit                                , outpostId        )          {
...
export function collectOutpostSupply(
  ...
  const nextTransit                     = transit ? { ...transit } : createTransitState();
  const nextCarried            = { ...carried };
