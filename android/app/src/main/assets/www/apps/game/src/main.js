...
export function getAvailableWeaponsList()              {
...
export function getCurrentEquippedWeapon()            {
...
export function cycleNextWeapon()       {
  const available = getAvailableWeaponsList();
...
export function updateCombatPadUI()       {
...
export function handleHuntBeast(beast                  , clickScreenX         , clickScreenY         )       {
  if (!app.profile) return;
  const nowMs = Date.now();
  const { render: playerAt } = currentPosition();
  const WORLD_ORIGIN_LAT = 21.0;
...
let lastTreasureVibratedId                = null;

function updateHanoiTreasureHud(playerPos        )       {
  if (!app.profile) return;
  const banner = document.getElementById('hanoi-treasure-hud-banner');
  if (!banner) return;

  const nowMs = Date.now();
  let clue                      = app.profile.activeTreasureClue ?? null;
...
function openHanoiTreasureModal()       {
  if (!app.profile) return;
...
function handleClaimHanoiTreasure()       {
...
function handleRefreshHanoiTreasure()       {
  if (!app.profile) return;
  const { render: playerAt } = currentPosition();
  const pack = sampleHanoiPack();
...
let currentBeastRadarId                = null;

function updateBeastHuntRadar(beast                  , dist        )       {
  const banner = document.getElementById('drop-radar-banner');
 ...
 */
function spawnSingleWorldDropNear(_center        , _zone        )       {
...
let virtualPlayerPos         = { lat: 21.0068, lon: 105.8431 };
let smoothRenderPos                = null;
let devMockPosition                = null;
let mapPickMode                                      = null;
let relocateTargetPos                = null;
...
function openRelocateCampModal(targetPos         )       {
  if (!app.profile) return;
...
function setupVirtualJoystick()       {
  ...
  const updateStickPos = (clientX        , clientY        ) => {
...
function setupCombatPad()       {
  ...
  const wireAimButton = (button                    , isSkill         ) => {
    if (!button) return;
    let pointerId                = null;
    let startX = 0;
    let startY = 0;
...
function enterProfile(slot        )       {
  ...
  let pendingCampPos                = null;

  function promptConfirmCampLocation(pos        )       {
    pendingCampPos = pos;
  ...
  function setupInitialCampAt(pos        )       {
    if (!app.profile) return;
...
function checkBeastAttackDamage(beast                                        )       {
  if (!app.profile) return;
...
function wireStaticControls()       {
  ...
  const openHeroProfile = () => {
    ...
    const btnUpgradeSpeed = document.getElementById('btn-hero-upgrade-speed')                            ;
