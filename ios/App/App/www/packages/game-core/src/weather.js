...
import { dayKey, toLocalTime } from './time.js';
 ...
 */
export function weatherFor(at        , nowMs        , offsetMinutes         )               {
  ...
  const rainIntensity = raining ? ((def                              ).rainIntensity ?? 1) : 0;
...
export function forecast(at        , nowMs        , days = 3, offsetMinutes         )                 {
  const out                 = [];
...
export function modifiersOf(weather              )                   {
...
export function applyRealWeather(
  ...
  return {
    ...
    conditionNameVi: def.nameVi,
    raining,
    rainIntensity: raining ? ((def                              ).rainIntensity ?? 1) : 0,
...
export function skyTint(nowMs        , weather              , offsetMinutes         )         {
