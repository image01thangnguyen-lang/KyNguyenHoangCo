...
import { toLocalTime, VN_UTC_OFFSET_MINUTES } from './time.js';
...
export function isWeekend(nowMs        , offsetMinutes = VN_UTC_OFFSET_MINUTES)          {
...
export function getWeekendKey(nowMs        , offsetMinutes = VN_UTC_OFFSET_MINUTES)         {
  const local = toLocalTime(nowMs, offsetMinutes);
...
export function claimWeekendQuest(
  ...
  const weekendKey = getWeekendKey(nowMs, offsetMinutes);
  const claimedMap = (player       ).weekendQuestsClaimed ?? {};
