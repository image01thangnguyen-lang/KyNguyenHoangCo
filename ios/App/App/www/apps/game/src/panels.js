...
export function renderSafeVaultSection(profile             , handlers          )       {
  ...
  const btnRelocate = box.querySelector                   ('#btn-open-relocate-camp');
  if (btnRelocate) {
...
export function renderBagPanel(profile             , handlers          )       {
  ...
  const btnUpgradeStrength = document.getElementById('btn-bag-upgrade-strength')                            ;
...
export function renderLog(
  ...
  if (weekendBoard.isWeekendActive) {
    ...
    for (const claimBtn of weekendCard.querySelectorAll                   ('.btn-claim-weekend')) {
      claimBtn.onclick = (e) => {
        e.stopPropagation();
