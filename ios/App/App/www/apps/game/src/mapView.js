...
import { BeastEntity } from './entities/beastEntity.js';
import { StructureEntity } from './entities/structureEntity.js';
import { WorldDropEntity } from './entities/worldDropEntity.js';
import { GameCamera } from './camera/gameCamera.js';
import { CombatVFXSystem } from './vfx/combatVFX.js';
import { WeatherOverlaySystem } from './vfx/weatherOverlays.js';
import { SpriteSheetAnimator,                  } from './animation/spriteSheetAnimator.js';
import {                       mapBeastSpeciesToCatalog, getCatalogEntry } from './animation/entityCatalog.js';
import { YSortManager } from './animation/ySortManager.js';
...
const ROAD_SPATIAL_INDEX = new Map                   ();
const roadGridKey = (latCell        , lonCell        )         => `${latCell}:${lonCell}`;
...
export class MapView {
  ...
  // HỆ THỐNG ASSET LOADER, CAMERA & HIỆU ỨNG THỊ GIÁC 2.5D
           ...
           combatVFX = new CombatVFXSystem();
           weatherOverlay = new WeatherOverlaySystem();
           playerEntity = new PlayerEntity('player');
           ySortManager = new YSortManager();
          beastAnimatorsMap = new Map                             ();
  ...
  // HỆ THỐNG QUẢN LÝ MẶT ĐẤT & DẤU CHÂN TIỀN SỬ
           footprintManager = new FootprintManager(300, 500);
          lastPlayerPos                = null;
          lastPlayerHeadingRad = 0;
          playerStealthState = { isStealthed: false, opacity: 1.0 };
          walkPhase = 0;
          lastFootPlant = 0;
          playerFacingAngle = 0;
          stepDustParticles                                                                                    = [];
          ...
          renderedFarmPlots                                                                                     = [];
          renderedStations                                                                     = [];
          renderedCampBounds                                                  = null;
          renderedBeasts                                                                           = [];

         onBeastClick                                                                        ;
         nearestAttackingBeast                                                                          = null;
          hitFlashAlpha = 0;
          hitDamageNumber                                                   = null;
          beastDamageNumbers                                                                                = [];
  ...
  // MẪU TEXTURE THẾ HỆ MỚI CHUẨN ĐẾ CHẾ AOE 1 (LUSH PROCEDURAL RETRO PIXEL PATTERNS)
          patternGrassDay                       = null;
          patternGrassNight                       = null;
          patternDirt                       = null;
          patternSteppe                       = null;
          patternForestFloor                       = null;

         isRainingNow = false;
         rainIntensityNow = 0;

  /** Sinh hạt bụi đất/cát văng nhẹ dưới chân khi di chuyển hoặc dẫm chân */
         spawnFootDust(x        , y        , count = 3, color = 'rgba(197, 160, 110, 0.45)')       {
    for (let i = 0; i < count; i++) {
      const angle = Math.random() * Math.PI * 2;
  ...
  // ================================================================
    ...
    const getOrCreateBeast = (id        , species              , ox        , oy        )                          => {
...
// ================================================================
  ...
  // ================================================================
    ...
    const targetState              = isChasing ? 'RUN' : (isMoving ? 'WALK' : 'IDLE');
    ...
    ctx.restore();
          ...
          drawTRex(sx        , sy        , pxPerMeter        , TILT_Y        , distToPlayerMeters         = 99, beast                          )       {
    this.renderBeastEntity(sx, sy, pxPerMeter, TILT_Y, distToPlayerMeters, 'beast_trex', 'Bạo Chúa T-Rex', '🦖', 12.5, 6.0, beast);
  }

          drawAnkylosaurus(sx        , sy        , pxPerMeter        , TILT_Y        , distToPlayerMeters         = 99, beast                          )       {
    this.renderBeastEntity(sx, sy, pxPerMeter, TILT_Y, distToPlayerMeters, 'beast_ankylosaurus', 'Khủng Long Thiết Giáp', '🛡️', 8.0, 4.8, beast);
          ...
          drawTitanoboa(sx        , sy        , pxPerMeter        , TILT_Y        , distToPlayerMeters         = 99, beast                          )       {
    ...
    this.renderBeastEntity(sx, sy, pxPerMeter, TILT_Y, distToPlayerMeters, 'beast_plesiosaur', 'Thủy Long Hồ Tây', '🐉', 10.0, 5.0, beast, { isAquatic: true });
          ...
          drawPterosaur(sx        , sy        , pxPerMeter        , TILT_Y        , distToPlayerMeters         = 99, beast                          )       {
    this.renderBeastEntity(sx, sy, pxPerMeter, TILT_Y, distToPlayerMeters, 'beast_pterosaur', 'Dực Long Bay', '🦅', 5.5, 4.0, beast, { isFlying: true });
          ...
          drawSarcosuchus(sx        , sy        , pxPerMeter        , TILT_Y        , distToPlayerMeters         = 99, beast                          )       {
  ...
  /** RỪNG THÔNG KIM 3 TẦNG ĐẾ CHẾ (AOE Pine Tree) */
          drawPineTree(sx        , sy        , pxPerMeter        , TILT_Y        , rand        )       {
    const { ctx } = this;
  ...
  /** CÂY SỒI CỔ THỤ TÁN TRÒN (AOE Oak Tree) */
          drawOakTree(sx        , sy        , pxPerMeter        , TILT_Y        , rand        )       {
    const { ctx } = this;
  ...
  /** BÃI QUẢ DÂU ĐỎ MỌNG (AOE Berry Forage Bushes) */
          drawBerryBush(sx        , sy        , pxPerMeter        , TILT_Y        , rand        )       {
    const { ctx } = this;
  ...
  /** MỎ ĐÁ TẢNG HOA CƯƠNG 3D (Stone Quarry Rocks) */
          drawStoneQuarry(sx        , sy        , pxPerMeter        , TILT_Y        , rand        )       {
    const { ctx } = this;
  ...
  /** BỘ XƯƠNG HÓA THẠCH KHỦNG LONG TIỀN SỬ (Dino Fossil Skeleton) */
          drawDinoSkeletonFossil(sx        , sy        , pxPerMeter        , TILT_Y        , rand        )       {
    const { ctx } = this;
  ...
  /** KHÓM CỎ & BỤI DƯƠNG XỈ 2.5D (Illustrated 2.5D Grass Tufts & Ferns) */
          drawGrassTuftsAndFern(sx        , sy        , pxPerMeter        , TILT_Y        , rand        )       {
    const { ctx } = this;
  ...
  // ================================================================
    ...
    const farmPlots = (camp?.farmPlots ?? [])              ;
  ...
  /** Vẽ tòa kiến trúc trung tâm căn cứ theo cấp độ và cột cờ hiệu kỳ */
    ...
    if (campLevel === 1) {
      ...
      const drawThatchTier = (topX        , topY        , botW        , tierH        , colorBase        , colorMid        , colorSun        ) => {
        const halfW = botW / 2;
   ...
   */
          drawDinoFootprintsAndParticles(
    w        ,
    h        ,
    input             ,
    project                                  ,
   ...
   */
          drawDinoStealthHud(w        , h        , playerScreenX        , playerScreenY        )       {
