...
import { BLOOD_MOON } from '../src/balance.ts';
import {
  allyDps,
  attackBoss,
  bloodMoonStatus,
  ...
  startBloodMoon,
  tickAllies,
} from '../src/bloodMoon.ts';
import { createCampState } from '../src/crafting.ts';
import { campDefensePower, forecastTonight, resolveNightDefense } from '../src/nightDefense.ts';
import { createRng } from '../src/rng.ts';
import { isBloodMoonWindow, isMakeupWindow } from '../src/time.ts';
import type { CampState } from '../src/types.ts';
...
const base = (): CampState => createCampState(SATURDAY_20H);

const fortress = (): CampState => ({
  ...base(),
  level: 3,
...
test('offline vẫn tự thủ nhưng yếu hơn có người canh (§5.4)', () => {
  const camp: CampState = { ...base(), level: 2, defenseStructures: { wooden_wall: 5, spike_trap: 3 } };
