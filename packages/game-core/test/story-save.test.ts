...
import type { QuestSnapshot } from '../src/story.ts';
...
test('nhiệm vụ đã nhận thưởng không nhận lại lần hai', () => {
  const state = createStoryState();
  const snap: QuestSnapshot = { ...emptySnapshot(), lifetimeCollected: { dry_branch: 8 } };
...
test('checksum ổn định bất kể thứ tự khoá trong object', () => {
  ...
  assert.equal(computeChecksum(a as never), computeChecksum(b as never));
});
