...
import type { ProfileSave } from '../src/save.ts';
...
function newProfile(nowMs = MONDAY_8H): ProfileSave {
  return createProfile('Người thử', nowMs, 'tester');
}

function open(profile: ProfileSave, nowMs: number, steps: number, position = HO_GUOM) {
