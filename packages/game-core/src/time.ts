...
import { BLOOD_MOON, NIGHT_DEFENSE, GATHERING } from './balance.ts';

export const VN_UTC_OFFSET_MINUTES = 7 * 60;

export interface LocalTime {
  /** YYYY-MM-DD theo giờ địa phương của người chơi. */
  day: string;
  hour: number;
  minute: number;
...
export function toLocalTime(ms: number, offsetMinutes = VN_UTC_OFFSET_MINUTES): LocalTime {
...
export function dayKey(ms: number, offsetMinutes = VN_UTC_OFFSET_MINUTES): string {
 ...
 */
export function phaseOf(ms: number, offsetMinutes = VN_UTC_OFFSET_MINUTES): Phase {
...
export function isNightDefenseWindow(ms: number, offsetMinutes = VN_UTC_OFFSET_MINUTES): boolean {
  const { hour } = toLocalTime(ms, offsetMinutes);
  return hour >= NIGHT_DEFENSE.windowStartHour && hour < NIGHT_DEFENSE.windowEndHour;
}

export function isBloodMoonWindow(ms: number, offsetMinutes = VN_UTC_OFFSET_MINUTES): boolean {
...
export function isMakeupWindow(ms: number, offsetMinutes = VN_UTC_OFFSET_MINUTES): boolean {
  const { hour, dayOfWeek } = toLocalTime(ms, offsetMinutes);
  const makeup = BLOOD_MOON.makeupFight;
...
export function outdoorPolicy(
  ...
  offsetMinutes = VN_UTC_OFFSET_MINUTES,
): { showSafetyWarning: boolean; poiLocked: boolean; reasonVi?: string } {
  const { hour } = toLocalTime(ms, offsetMinutes);
