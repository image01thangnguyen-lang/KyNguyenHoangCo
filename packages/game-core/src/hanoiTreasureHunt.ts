...
import type { LatLon, Poi } from './world.ts';
import type { Inventory, ProfileSave } from './save.ts';

export type TreasureRewardTier = 'common_cache' | 'rare_military' | 'ancient_relic' | 'royal_treasure';
...
export interface TreasureClue {
  ...
  targetLat: number;
  targetLon: number;
  /** Khoảng cách ước tính lúc phát lệnh (mét) */
  initialDistanceMeters: number;
  /** Thể loại kho báu */
  rewardTier: TreasureRewardTier;
  rewardTitleVi: string;
  rewardDescriptionVi: string;
  /** Danh sách vật phẩm thưởng */
  rewards: Inventory;
  /** Điểm thưởng danh hiệu */
  memoryScore: number;
  /** Thời điểm tạo */
  createdAtMs: number;
  /** Hạn chót (ví dụ 48 tiếng) */
  expiresAtMs: number;
}

/** Danh hiệu Thổ Địa Hà Thành theo số lượng kho báu đã tìm thấy */
export function getHanoiExplorerTitle(claimedCount: number): { titleVi: string; rank: number; badgeEmoji: string } {
...
export function generateHanoiTreasureClue(
  ...
  let rewardTier: TreasureRewardTier = 'common_cache';
...
export interface ClaimResult {
  ok: boolean;
  messageVi: string;
  claimedClue?: TreasureClue;
  gainedRewards?: Inventory;
  totalScore?: number;
  newExplorerTitle?: string;
}
