...
export interface LunarEventConfig {
  id: string;
  nameVi: string;
  descVi: string;
  startMonth: number;
  ...
  endMonth: number;
  endDay: number;
  campDecoration: string;
  harvestMultiplier: number;
  incubationMultiplier: number;
  campDefenseBonus: number;
  specialDropItemId: string;
  specialDropQty: number;
}
 ...
 */
export function convertSolarToLunar(solarDay: number, solarMonth: number, solarYear: number, timeZone = 7): LunarDate {
 ...
 */
export function getActiveLunarEvent(nowMs: number): (LunarEventConfig & { lunarDate: LunarDate }) | null {
  const date = new Date(nowMs);
  const lunar = convertSolarToLunar(date.getDate(), date.getMonth() + 1, date.getFullYear());

  const events = lunarEventsData.events as LunarEventConfig[];
