...
export interface StepFilterInput {
  /** Bước thô do cảm biến báo kể từ lần đồng bộ trước. */
  rawNewSteps: number;
  /** Thời gian đã trôi qua kể từ lần đồng bộ trước, ms. */
  elapsedMs: number;
  /**
   * Khoảng cách giữa các bước liên tiếp (ms) nếu tầng cảm biến cung cấp được.
   * Máy lắc điện thoại cho dãy này đều tăm tắp; người đi bộ thì không.
   */
  stepIntervalsMs?: number[];
}

export interface StepFilterResult {
  /** Số bước được nhận để tính thưởng ngay bây giờ. */
  accepted: number;
  /** Số bước hoãn lại cho lần đồng bộ sau (KHÔNG mất). */
  deferred: number;
  /** Số bước bị loại hẳn — chỉ xảy ra khi phát hiện mẫu máy lắc. */
  rejected: number;
  flaggedShake: boolean;
  flaggedCadence: boolean;
  noteVi?: string;
}

export function filterSteps(input: StepFilterInput): StepFilterResult {
 ...
 */
export function looksLikeShaking(intervalsMs: number[]): boolean {
 ...
 */
export function rewardableToday(rewardedSoFar: number, newSteps: number): {
  rewardable: number;
  overCap: number;
  capReached: boolean;
} {
