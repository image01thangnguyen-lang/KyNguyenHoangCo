import type { ProfileSave, ItemId } from './types.ts';
import { getPetCombatBonus } from './pets.ts';
...
export interface CoopBoss {
  ...
  maxHp: number;
  attackPower: number;
  specialSkillVi: string;
}

export type CoopActionType = 'attack' | 'reinforce' | 'heal_team' | 'arrow_volley';

export interface CoopPlayerAction {
  peerId: string;
  action: CoopActionType;
  targetPeerId?: string;
}
...
export function createCoopRoom(
  roomId: string,
  hostPeerId: string,
  hostProfile: ProfileSave,
  difficulty: 'normal' | 'hard' | 'nightmare' = 'normal',
  nowMs: number = Date.now(),
): CoopRoom {
  const hostPet = hostProfile.player.pets?.find((p) => p.isActive)?.petId ?? null;
  const hostWeapon: ItemId | null = hostProfile.player.inventory.find((i) => i.itemId.includes('spear') || i.itemId.includes('axe') || i.itemId.includes('bow'))?.itemId ?? null;

  const hostMember: CoopMember = {
    peerId: hostPeerId,
...
export function joinCoopRoom(
  ...
  const weapon: ItemId | null = profile.player.inventory.find((i) => i.itemId.includes('spear') || i.itemId.includes('axe') || i.itemId.includes('bow'))?.itemId ?? null;

  const newMember: CoopMember = {
    peerId,
 ...
 */
export function startCoopBattle(room: CoopRoom): CoopRoom {
 ...
 */
export function resolveCoopRewards(room: CoopRoom): CoopReward[] {
