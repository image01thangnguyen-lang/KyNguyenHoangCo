...
const overlay = () => document.getElementById('overlay-action') as HTMLElement;
const card = () => document.getElementById('action-card') as HTMLElement;

/** Thanh nhịp một lượt. Trả về điểm 0..1. Dùng chung cho câu cá và cho các trận đánh. */
export function timingRound(host: HTMLElement, speed = 1): Promise<number> {
  return new Promise((resolve) => {
    ...
    const cursor = bar.querySelector<HTMLElement>('.timing__cursor')!;
 ...
 */
function tapRhythm(host: HTMLElement, durationSeconds: number, onTick: (text: string) => void): Promise<number> {
  return new Promise((resolve) => {
    const beatMs = 1100;
    ...
    const axe = stage.querySelector<HTMLElement>('.chop__axe')!;
    const start = performance.now();
    let scored = 0;
    let hits = 0;
 ...
 */
export function openMinigame(titleVi: string, spec: MinigameSpec): Promise<number | null> {
  return new Promise((resolve) => {
