...
function recipeRow(entry: RecipeView, profile: ProfileSave, handlers: Handlers): HTMLElement {
  const row = document.createElement('div');
...
export function renderBagPanel(profile: ProfileSave, handlers: Handlers): void {
  ...
  for (const [itemId, qty] of entries) {
    ...
    if (isTrap) {
      ...
      trapBtn.onclick = () => handlers.onPlaceTrap(itemId as 'rabbit_trap' | 'deer_trap' | 'beast_trap');
      card.append(trapBtn);
    }
